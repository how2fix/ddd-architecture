 ...
 */
@Slf4j
@Extension(bizId = "DEFAULT", useCase = "withdraw", description = "提现场景风控-普通用户")
public class WithdrawRiskCheckExt implements RiskCheckExtPt {

    // 普通用户单日限额5000元
    private static final BigDecimal DAILY_LIMIT = new BigDecimal("5000");
    private static final BigDecimal SINGLE_LIMIT = new BigDecimal("2000");

    @Override
    public RiskCheckResult check(Long userId, BigDecimal amount, RiskContext context) {
        log.debug("执行提现风控检查(普通用户): userId={}, amount={}", userId, amount);

        // 检查单笔限额
        if (amount.compareTo(SINGLE_LIMIT) > 0) {
            return RiskCheckResult.reject(
                    String.format("普通用户单笔提现限额%s元", SINGLE_LIMIT));
        }
